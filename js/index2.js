...
function rel_html_imgpath(iconurl)
    {
        console.log(canonical_uri(iconurl.replace(/.*\/([^\/]+\/[^\/]+)$/, '$1')));
...
function showInfo() {
    ...
    if (theme == "light") {
        loadExtentFile(rel_html_imgpath(__uri("../css/light.css")), "css");
    }
    else{
        loadExtentFile(rel_html_imgpath(__uri("../css/index.css")), "css");
    }
